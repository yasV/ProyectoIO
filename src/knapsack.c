...
const char *rowHeader[4] = {"","Costo","Valor","Cantidad Disponible"};
...
void createTableData(int totalObjects) {
...
void on_btn_getEntries_clicked() {
	...
	printf("La Capacidad de la mochila es: %d y habran %d tipos de objetos \n", entry_knapsackCapacity, 
		entry_totalObjects);
	createTableData(entry_totalObjects + 1);
...
void on_btn_getTableData_clicked() {
	printf("Nombre del archivo a generar: %s\n", gtk_entry_get_text (GTK_ENTRY(g_entry_fileName)));
}